...
`POST /api/v1/facebook/{page_uuid}/posts`
...
```bash [cURL]
curl -X POST https://api.callmissed.com/v1/facebook/PAGE_UUID/posts \
...
`POST /api/v1/facebook/{page_uuid}/photos`
...
`POST /api/v1/facebook/{page_uuid}/reels`
...
`POST /api/v1/instagram/{ig_account_uuid}/posts`
...
```bash [cURL]
# A 3-image carousel
curl -X POST https://api.callmissed.com/v1/instagram/IG_UUID/posts \
  -H "Authorization: Bearer cm_your_key" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "carousel",
...
**Check a container's status** — `GET /api/v1/instagram/{ig_account_uuid}/posts/{container_id}`

Poll this until the status is `FINISHED`, then publish. A container **expires 24
hours** after creation.

**Publish a finished container** — `POST /api/v1/instagram/{ig_account_uuid}/posts/{container_id}/publish`
