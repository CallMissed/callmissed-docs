...
A text reply into an existing thread. All three fields are required.
...
| Field | Type | Description |
|---|---|---|
| `ig_user_id` | string | The connected Instagram account's id (the thread's `account_ref`). |
| `recipient_id` | string | The recipient's id — the thread's `contact_id`. |
| `text` | string | The message text. |

```bash [cURL]
curl -X POST https://api.callmissed.com/v1/instagram/messages \
...
window was never opened), `400` when the window has closed, and `502` when the
upstream send fails.
