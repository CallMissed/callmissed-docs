...
description: "See which models each plan tier can call. Free plan includes 24 models across LLM, STT, TTS, and Image."
slug: "models-access"
breadcrumb: "LLM & AI"
---
...
See which models each plan tier can call. Free plan includes 24 models across LLM, STT, TTS, and Image.

## Overview

Not all models are available on every plan. The free tier includes 24 models; paid plans (Starter, Pro, Enterprise) unlock all models.
...
The free plan includes 24 models. Call `GET /v1/models` to fetch the current free-tier list programmatically (each model's `free` flag).
...
Paid-only models include: all first-party IDs (`gpt-4o`, `gpt-4.1`, `gpt-5-mini`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `grok-4.3`, `DeepSeek-V4-Pro`, `DeepSeek-V4-Flash`, `gpt-realtime`, `gpt-realtime-mini`, `gpt-realtime-2`, `gpt-realtime-1.5`, `gpt-realtime-2.1`, `gpt-realtime-2.1-mini`, `nova-sonic-2`, `nova-sonic`, `whisper`, `gpt-4o-transcribe`, `gpt-4o-mini-transcribe`, `gpt-4o-transcribe-diarize`, `gpt-4o-mini-tts`), Deepgram direct + voice-agent-only ids (`deepgram-*`, including `deepgram-voice-*`), `kimi-k2.5-fast` (under maintenance), slash-prefixed frontier IDs (`google/gemini-*`, `openai/*`, `anthropic/*`, `x-ai/*`, `qwen/*`, `mistralai/*` and the rest of the passthrough catalog), and paid image models (`flux-2-pro`, `flux-1.1-pro`, `gpt-image-2`, `gpt-image-1.5`, `nano-banana-2`, `nano-banana-pro`). See [Full Model Catalog](/docs/models#full-model-catalog) for all 125 models.
...
{
  "error": {
    "message": "Model 'gpt-5.6-terra' requires a paid plan. See GET /api/v1/models/access for the full list of free-plan models. Upgrade at https://app.callmissed.com/pricing",
