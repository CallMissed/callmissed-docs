...
Nova 2 Sonic handles speech understanding, reasoning, turn-taking, function calling, and speech output in one model. If AWS Bedrock credentials are unavailable, the agent falls back to the cascaded STT→LLM→TTS stack automatically so sessions still connect.
