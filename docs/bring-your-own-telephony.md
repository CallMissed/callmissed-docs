...
> **Outbound calls authenticate with the SIP username and password, not with an IP allowlist.** Our media path does not present a single fixed egress IP, so an IP-only trunk cannot be authorised. Ask your provider to enable digest (username and password) authentication on the trunk. If they cannot, write to `support@callmissed.com` before you start.
...
- **Credentials are encrypted at rest with AES-256-GCM** before they reach the database. They are decrypted only in memory, only when a call to your provider needs them.
